...
STAGES = [("reading_video", 0.15), ("sampling_frames", 0.30), ("cv_inference", 0.80), ("assessing_quality", 0.90), ("finalizing", 0.95)]
...
async def run_analysis_job(db, job_id: str) -> None:
    ...
    try:
        ...
        try:
            ...
            attempts = int(job.get("attempts", 1)); max_attempts = int(os.environ.get("AI_COACH_JOB_MAX_ATTEMPTS", "5")); status = "queued" if attempts < max_attempts else "failed"
            await set_stage("queued" if status == "queued" else "failed", 0.0 if status == "queued" else 1.0, status=status, error=str(exc)[:300], last_error_at=_iso(), locked_at=None, locked_by=None); return
        for stage, p in STAGES[3:]: await set_stage(stage, p)
        analytics_doc: Dict[str, Any] = {"match_id": job["match_id"], "video_id": job["video_id"], "user_id": job["user_id"], "sport": job.get("sport", "pickleball"), "data_quality": result.data_quality.model_dump(), "metrics": [m.model_dump() for m in result.metrics], "rallies": result.rallies, "shots": result.shots, "important_moments": result.important_moments, "analyzer": result.analyzer, "analyzer_version": result.analyzer_version, "diagnostics": result.diagnostics, "generated_at": _iso()}
