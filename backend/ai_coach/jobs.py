...
log=logging.getLogger("kuvira.jobs"); WORKER_ID=f"{socket.gethostname()}:{os.getpid()}"; LOCK_TIMEOUT_SEC=int(os.environ.get("AI_COACH_JOB_LOCK_TIMEOUT_SEC","900"))
...
async def _claim_job(db,job_id:str)->dict|None:
    ...
    return await db.ai_coach_jobs.find_one_and_update({"id":job_id,"status":{"$in":["queued","processing"]},"$or":[{"locked_at":None},{"locked_at":{"$exists":False}},{"lock_epoch":{"$lt":stale_before}}]},{"$set":{"status":"processing","stage":"processing","started_at":_iso(),"locked_at":_iso(),"lock_epoch":now.timestamp(),"locked_by":WORKER_ID},"$inc":{"attempts":1}},projection={"_id":0},return_document=ReturnDocument.AFTER)
async def run_analysis_job(db,job_id:str)->None:
    ...
    try:
        ...
        try: result=await AsyncRetry(attempts=int(os.environ.get("AI_COACH_JOB_RETRIES","3"))).run(execute)
        ...
        await db.ai_coach_analytics.update_one({"match_id":job["match_id"],"user_id":job["user_id"]},{"$set":analytics_doc},upsert=True); await db.ai_coach_jobs.update_one({"id":job_id,"locked_by":WORKER_ID},{"$set":{"status":"completed","stage":"completed","progress":1.0,"completed_at":_iso(),"diagnostics":result.diagnostics,"locked_at":None,"locked_by":None}})
    except Exception as exc: await set_stage("failed",1.0,status="failed",completed_at=_iso(),error=str(exc)[:300],locked_at=None,locked_by=None)
    finally:
        if temp_path:
