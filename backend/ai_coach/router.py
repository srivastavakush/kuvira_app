...
def _get_queue():
    global _QUEUE
    if _QUEUE is None: _QUEUE = JobQueue()
    return _QUEUE

async def _allow_chat(user_id: str) -> bool:
    global _CHAT_DISTRIBUTED
    if os.environ.get("AI_COACH_RATE_LIMIT_BACKEND", "process").lower() != "mongo": return _CHAT_LIMITER.allow(user_id)
    if _CHAT_DISTRIBUTED is None: _CHAT_DISTRIBUTED = MongoRateLimiter(db, limit=_CHAT_LIMIT, window_seconds=60)
    return await _CHAT_DISTRIBUTED.allow(user_id)

async def _allow_analysis(user_id: str) -> bool:
    global _ANALYZE_DISTRIBUTED
    if os.environ.get("AI_COACH_RATE_LIMIT_BACKEND", "process").lower() != "mongo": return _ANALYZE_LIMITER.allow(user_id)
    if _ANALYZE_DISTRIBUTED is None: _ANALYZE_DISTRIBUTED = MongoRateLimiter(db, limit=_ANALYZE_LIMIT, window_seconds=3600)
...
async def _ensure_indexes():
    ...
    await db.ai_coach_analytics.create_index([("match_id", 1), ("user_id", 1)], unique=True)
    await db.ai_coach_reports.create_index([("match_id", 1), ("user_id", 1)], unique=True)
...
async def list_matches(user=Depends(current_user)):
    ...
    for d in docs:
        d["report"]=await db.ai_coach_reports.find_one({"match_id":d["id"],"user_id":user["id"]},{"_id":0,"id":1,"generated_at":1}); d["job"]=await db.ai_coach_jobs.find_one({"match_id":d["id"],"user_id":user["id"]},{"_id":0},sort=[("created_at",-1)])
    return docs
...
async def upload_video(file: UploadFile=File(...), match_id: Optional[str]=Form(None), user=Depends(current_user)):
    ...
    if file.headers and file.headers.get("content-length") and int(file.headers["content-length"]) > MAX_VIDEO_BYTES: raise HTTPException(413,"Video exceeds configured limit")
    video_id=gen_id()
    try: stored=await asyncio.to_thread(_get_storage().put, file.file, video_id, ext or ".mp4", MAX_VIDEO_BYTES)
    except ValueError: raise HTTPException(413,"Video exceeds configured limit")
    except Exception as exc: log.exception("video storage failed"); raise HTTPException(503,f"Video storage unavailable: {str(exc)[:120]}")
    doc={"id":video_id,"user_id":user["id"],"match_id":match_id,"original_filename":file.filename or "video","mime_type":file.content_type,"size_bytes":int(stored.get("size_bytes",0)),"storage":stored,"storage_backend":stored.get("backend"),"created_at":_iso()}
...
async def _load_context(user_id,match_id):
    profile=await db.users.find_one({"id":user_id},{"_id":0}) or {}; analytics=await db.ai_coach_analytics.find_one({"match_id":match_id,"user_id":user_id},{"_id":0}) or {}; history=await db.ai_coach_matches.find({"user_id":user_id},{"_id":0}).sort("created_at",-1).to_list(5); return {"player_profile":profile,"match_analytics":analytics,"recent_match_history":history}
...
async def player_performance(user=Depends(current_user)):
    ...
    for m in matches:
        an=await db.ai_coach_analytics.find_one({"match_id":m["id"],"user_id":user["id"]},{"_id":0})
        if an: entries.append({"match_id":m["id"],"created_at":m["created_at"],"metrics":an.get("metrics",[]),"data_quality":an.get("data_quality",{}),"rallies":an.get("rallies",[]),"shots":an.get("shots",[]),"points":an.get("points",[])})
