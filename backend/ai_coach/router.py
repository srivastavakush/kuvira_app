...
async def get_match_report(match_id:str,refresh:bool=False,user=Depends(current_user)):
    ...
    await db.ai_coach_reports.update_one({"match_id":match_id},{"$set":report},upsert=True); await db.ai_coach_matches.update_one({"id":match_id},{"$set":{"report_id":report["id"]}}); return report
...
async def chat(body:ChatBody,user=Depends(current_user)):
    ...
    try:
        result=await _get_chat_workflow().run_chat(user_id=user["id"],message=body.text,session_id=sid,match_id=body.match_id,sport="pickleball")
    except Exception as e:
        log.exception("agentic chat failed"); raise HTTPException(502,f"Coach model unavailable: {str(e)[:120]}")
    return result
@router.get("/history")
