...
class CoachingStateService:
    ...
    async def get_state(self,user_id:str)->Dict[str,Any]:
        await self.ensure_indexes(); doc=await self.db.ai_coach_player_state.find_one({"user_id":user_id},{"_id":0})
        if doc: return doc
    ...
    async def evolve_from_report(self,user_id:str,match_id:str,report:Dict[str,Any],data_quality:Optional[Dict[str,Any]]=None)->Dict[str,Any]:
        ...
        recurring=_unique_strings(previous_weaknesses+weaknesses); improving=_unique_strings(previous_improving+strengths); focus=_unique_strings(weaknesses[:3]+previous_weaknesses[:2],5); regressions=_unique_strings([w for w in weaknesses if _norm(w) in {_norm(x) for x in previous_weaknesses}],5); assignments=[]
