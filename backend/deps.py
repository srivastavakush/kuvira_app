...
# ---------------------------------------------------------------------------
# Config (env-driven; production values come from Secret Manager via env)
# ---------------------------------------------------------------------------
...
# OTP provider (Twilio Verify) — only required in production
OTP_PROVIDER = os.environ.get("OTP_PROVIDER", "mock")        # mock | twilio
...
# ---------------------------------------------------------------------------
# Structured logging (never log secrets/OTP/JWT/payment data)
# ---------------------------------------------------------------------------
...
# Permission catalog
PERM = {
    # club-scoped operational permissions
    "club.view": "View club workspace",
    "club.manage": "Edit club profile",
    "club.courts.manage": "Manage courts & availability",
    "club.bookings.manage": "Manage club bookings",
    ...
    "club.staff.manage": "Add/remove managers & staff",
    "club.ownership.transfer": "Transfer club ownership",
    "club.analytics.view": "View club analytics",
...
ROLE_PERMISSIONS: Dict[str, List[str]] = {
    ...
    ROLE_CLUB_STAFF: [
        "club.view", "club.bookings.manage", "club.games.manage",
    ],
    ROLE_PLATFORM_ADMIN: list(PERM.keys()),  # platform admin has everything
}
...
def make_token(user_id: str) -> str:
    now = utcnow()
    payload = {"sub": user_id, "iat": int(now.timestamp()), "exp": int((now + timedelta(days=JWT_EXPIRY_DAYS)).timestamp())}
...
async def _load_capabilities(user: dict) -> dict:
    """Attach roles/memberships/permissions determined by the BACKEND only."""
    ...
    for m in memberships:
        ...
        if org:
            orgs.append({"org_id": org["id"], "name": org["name"], "city": org.get("city"), "logo": org.get("logo"), "role": role})
    roles = [ROLE_PLAYER] + [m["role"] for m in memberships]
    if is_platform_admin:
        roles.append(ROLE_PLATFORM_ADMIN)
        perms |= set(PERM.keys())
    return {
...
async def current_user(authorization: Optional[str] = Header(None)) -> dict:
    if not authorization or not authorization.startswith("Bearer "):
        raise KuviraError(401, "UNAUTHENTICATED", "Missing or invalid Authorization header")
    token = authorization.split(" ", 1)[1]
    try:
        payload = decode_token(token)
    except jwt.ExpiredSignatureError:
        raise KuviraError(401, "TOKEN_EXPIRED", "Session expired, please sign in again")
    except jwt.PyJWTError:
        raise KuviraError(401, "TOKEN_INVALID", "Invalid token")
    user = await db.users.find_one({"id": payload["sub"]}, {"_id": 0})
    if not user:
        raise KuviraError(401, "USER_NOT_FOUND", "User not found")
...
async def current_capabilities(user: dict = Depends(current_user)) -> dict:
    return await _load_capabilities(user)
...
def require_platform_admin():
    async def _dep(user: dict = Depends(current_user)) -> dict:
        if not user.get("is_platform_admin"):
            raise KuviraError(403, "FORBIDDEN", "Platform admin access required")
        return user
    return _dep
...
async def get_membership(user_id: str, org_id: str) -> Optional[dict]:
    return await db.organization_memberships.find_one(
        {"user_id": user_id, "org_id": org_id, "status": "active"}, {"_id": 0}
    )


def require_org_permission(permission: str):
    """Ensure the caller has `permission` within the org identified by path param `org_id`.

    Platform admins bypass org membership. Enforced fully on the backend.
    """
    async def _dep(org_id: str, user: dict = Depends(current_user)) -> dict:
        ...
        if not m:
            raise KuviraError(403, "ORG_ACCESS_DENIED", "You are not a member of this organization")
        allowed = ROLE_PERMISSIONS.get(m.get("role"), [])
        if permission not in allowed:
            raise KuviraError(403, "PERMISSION_DENIED", f"Missing permission: {permission}")
        return user
    return _dep
