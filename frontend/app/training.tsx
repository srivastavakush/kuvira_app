  ...
  return (
    <SafeAreaView style={styles.wrap} testID="training-screen">
      <View style={styles.header}>
        ...
        <View style={styles.streakPill}><Ionicons name="flame" size={13} color={colors.warning} /><Text style={styles.streakText}>{streak.streak_days}d</Text></View>
      </View>

      <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.xxxl }} showsVerticalScrollIndicator={false}>
        <View style={styles.generator}>
          <Text style={styles.genLabel}>Build a training plan</Text>
          ...
          <Pressable testID="training-create-btn" style={styles.genBtn} disabled={creating} onPress={createPlan}>
            <Text style={styles.genBtnText}>{creating ? 'Building…' : `Create ${goal.toLowerCase()} plan`}</Text>
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  streakPill: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.surfaceSecondary, paddingHorizontal: spacing.md, height: 32, borderRadius: radius.pill },
  streakText: { color: colors.onSurface, fontWeight: '700', fontSize: font.sizes.sm },
  ...
  weekLabel: { color: colors.onSurfaceMuted, fontSize: font.sizes.xs, textTransform: 'uppercase', letterSpacing: 1, fontWeight: '700', marginBottom: spacing.sm },
