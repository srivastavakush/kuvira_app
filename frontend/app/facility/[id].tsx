    ...
    <View style={styles.wrap} testID="facility-detail-screen">
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          <Image source={{ uri: f.image }} style={StyleSheet.absoluteFillObject} contentFit="cover" />
          ...
          <SafeAreaView edges={['top']}>
            <Pressable testID="facility-back" onPress={() => router.back()} style={styles.backBtn}>
          ...
          <View style={styles.heroBottom}>
            ...
            <Text style={styles.loc}>📍 {f.area}, {f.city} · ⭐ {f.rating} ({f.reviews_count})</Text>
          </View>
        </View>
        ...
        <View style={styles.body}>
          ...
          <Text style={styles.sectionH}>Amenities</Text>
          ...
          <Text style={styles.sectionH}>Details</Text>
          <View style={styles.detailGrid}>
            <View style={styles.detailBox}><Text style={styles.detailVal}>{f.courts_count}</Text><Text style={styles.detailLabel}>Courts</Text></View>
            <View style={styles.detailBox}><Text style={styles.detailVal}>₹{f.price_per_hour}</Text><Text style={styles.detailLabel}>Per hour</Text></View>
            <View style={styles.detailBox}><Text style={styles.detailVal}>{f.rating}</Text><Text style={styles.detailLabel}>Rating</Text></View>
          </View>
        </View>
      </ScrollView>
      ...
      <View style={styles.footer}>
        <View>
          <Text style={styles.footerPrice}>₹{f.price_per_hour}<Text style={styles.footerUnit}>/hr</Text></Text>
          <Text style={styles.footerSub}>+ taxes</Text>
        </View>
        <Pressable testID="facility-book-btn" style={styles.bookBtn} onPress={() => router.push(`/booking/${f.id}`)}>
...
const styles = StyleSheet.create({
  ...
  loc: { color: colors.onSurfaceSecondary, fontSize: font.sizes.base, marginTop: 4 },
  ...
  amenity: { backgroundColor: colors.surfaceSecondary, borderWidth: 1, borderColor: colors.border, paddingHorizontal: spacing.md, paddingVertical: 8, borderRadius: radius.pill },
  amenityText: { color: colors.onSurfaceSecondary, fontSize: font.sizes.sm },
  detailGrid: { flexDirection: 'row', gap: spacing.md },
  detailBox: { flex: 1, backgroundColor: colors.surfaceSecondary, borderRadius: radius.md, padding: spacing.md, alignItems: 'center', borderWidth: 1, borderColor: colors.border },
  detailVal: { color: colors.brandPrimary, fontSize: font.sizes.xxl, fontWeight: '900' },
  detailLabel: { color: colors.onSurfaceMuted, fontSize: font.sizes.xs, marginTop: 2, textTransform: 'uppercase', letterSpacing: 1 },
  ...
  footerUnit: { color: colors.onSurfaceMuted, fontSize: font.sizes.base, fontWeight: '600' },
  footerSub: { color: colors.onSurfaceMuted, fontSize: font.sizes.xs },
  bookBtn: { backgroundColor: colors.brandPrimary, paddingHorizontal: spacing.xxl, paddingVertical: spacing.md, borderRadius: radius.pill },
  bookBtnText: { color: colors.onBrandPrimary, fontSize: font.sizes.lg, fontWeight: '800' },
});
